...
import {
  ...
  Download, FileJson, FileText, FileSpreadsheet, Printer, ArrowLeft, Save, AlertCircle,
} from 'lucide-react';
...
function dateOrd(d: string): number {
...
// ── Reorder helpers ─────────────────────────────────────
function insertWithReorder(item: Chamada, list: Chamada[]): Chamada[] {
...
function reorderAfterDelete(deletedOrd: number, list: Chamada[]): Chamada[] {
  return list
    .map(m => m.ord > deletedOrd ? { ...m, ord: m.ord - 1 } : m)
    .sort((a, b) => a.ord - b.ord);
}

function reorderAfterMove(item: Chamada, oldOrd: number, list: Chamada[]): Chamada[] {
  const newOrd = item.ord;
  const reordered = list.map(m => {
    if (newOrd < oldOrd) return m.ord >= newOrd && m.ord < oldOrd ? { ...m, ord: m.ord + 1 } : m;
...
const emptyForm = (): Omit<Chamada, 'id'> => ({
  ord: 0, posto: 'SD PM', rg: '', nome: '',
  dataInclusao: todayBR(), cpf: '', celular: '', whatsapp: '', endereco: '',
});

interface Errors { cpf?: string; nome?: string; celular?: string; }

export default function PlanoChamadaModule({ onBack, permissions }: { onBack: () => void; permissions?: ModulePermission }) {
  ...
  const [modal, setModal] = useState<{ mode: ModalMode; item: Chamada | null } | null>(null);
  const [form, setForm] = useState<Omit<Chamada, 'id'>>(emptyForm());
  const [errors, setErrors] = useState<Errors>({});
  const [deleteTarget, setDeleteTarget] = useState<Chamada | null>(null);
  ...
  const filtered = useMemo(() => {
    ...
    return list.sort((a, b) => {
      let cmp = sortKey === 'posto'
        ...
        : sortKey === 'ord'
        ? a.ord - b.ord
        : sortKey === 'dataInclusao'
        ? dateOrd(a.dataInclusao) - dateOrd(b.dataInclusao)
  ...
  // ── Form helpers ────────────────────────────────────
  const changeField = (key: keyof Omit<Chamada, 'id'>, raw: string) => {
    let value = raw;
    if (key === 'cpf') value = maskCPF(raw);
    if (key === 'celular' || key === 'whatsapp') value = maskPhone(raw);
  ...
  const validate = (): boolean => {
    ...
    if (cpfDigits.length > 0 && !validateCPF(form.cpf)) errs.cpf = 'CPF inválido';
    const telDigits = form.celular.replace(/\D/g, '');
    if (telDigits.length > 0 && telDigits.length < 10) errs.celular = 'Celular incompleto';
    setErrors(errs);
  ...
  const openView = (m: Chamada) => { setForm(m); setErrors({}); setModal({ mode: 'view', item: m }); };
  const openEdit = (m: Chamada) => { setForm(m); setErrors({}); setModal({ mode: 'edit', item: m }); };
  ...
  const handleSave = () => {
    ...
    if (modal?.mode === 'create') {
      const item: Chamada = { ...form, id: Date.now().toString() };
      setData(d => insertWithReorder(item, d));
    } else if (modal?.mode === 'edit' && modal.item) {
      const item: Chamada = { ...form, id: modal.item.id };
  ...
  const exportCSV = () => {
    const rows = data.map(m =>
      `${m.ord},"${m.posto}","${m.rg}","${m.nome}","${m.dataInclusao}","${m.cpf}","${m.celular}","${m.whatsapp}","${m.endereco}"`
    ).join('\n');
    dl('plano_chamada.csv', 'text/csv;charset=utf-8', '﻿Nº,Posto/Grad.,RG,Nome,Data Inclusão,CPF,Celular,WhatsApp,Endereço\n' + rows);
  };

  const exportJSON = () => dl('plano_chamada.json', 'application/json', JSON.stringify(data, null, 2));
  ...
  const exportXLS = () => {
    const rows = data.map(m => ({
      ...
      'RG': m.rg,
      'Nome': m.nome,
      'Data de Inclusão': m.dataInclusao,
      'CPF': m.cpf,
      'Celular': m.celular,
      'WhatsApp': m.whatsapp,
      'Endereço': m.endereco,
    ...
    ws['!cols'] = [
      { wch: 5 }, { wch: 14 }, { wch: 8 }, { wch: 38 }, { wch: 14 },
      { wch: 16 }, { wch: 16 }, { wch: 16 }, { wch: 60 },
    ];
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Plano de Chamada');
    XLSX.writeFile(wb, 'plano_chamada.xlsx');
    setShowExport(false);
  };

  const exportPrint = () => {
    const rows = data.map(m =>
      `<tr><td>${m.ord}</td><td>${m.posto}</td><td>${m.rg}</td><td>${m.nome}</td><td>${m.dataInclusao}</td><td>${m.cpf}</td><td>${m.celular}</td><td>${m.endereco}</td></tr>`
    ).join('');
    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Plano de Chamada 31ª CIPM-CPE</title>
      <style>body{font-family:Arial;font-size:10px}table{width:100%;border-collapse:collapse}
      th,td{border:1px solid #ccc;padding:4px 6px}th{background:#333;color:#fff}
      h3{margin-bottom:10px}</style></head><body>
      <h3>Plano de Chamada — 31ª CIPM-CPE</h3>
      <table><thead><tr><th>Nº</th><th>Posto/Grad.</th><th>RG</th><th>Nome</th><th>Data Inclusão</th><th>CPF</th><th>Celular</th><th>Endereço</th></tr></thead>
    ...
    <div className="flex flex-col h-full">
        ...
        <div className="relative">
          ...
          {showExport && (
            ...
            >
              ...
              <button onClick={exportXLS} className="adm-drop-item flex items-center gap-3 w-full px-4 py-3 text-base transition-colors" style={{ color: 'var(--adm-text)' }}>
                <FileSpreadsheet size={16} className="text-green-500" /> Excel (XLS)
              </button>
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          placeholder="Buscar por nome, posto, RG, celular ou endereço..."
        ...
        <table className="w-full">
          <thead className="sticky top-0 z-10" style={{ background: 'var(--adm-tbl-head)' }}>
            <tr className="text-sm font-semibold uppercase tracking-wide" style={{ color: 'var(--adm-muted)' }}>
              <th className="px-4 py-3.5 text-left cursor-pointer hover:opacity-100 opacity-80 select-none w-14" onClick={() => toggleSort('ord')}>
                # <SortIcon col="ord" />
              </th>
              ...
              <th className="px-4 py-3.5 text-left cursor-pointer hover:opacity-100 opacity-80 select-none w-24 hidden sm:table-cell" onClick={() => toggleSort('rg')}>
                RG <SortIcon col="rg" />
              </th>
              ...
              <th className="px-4 py-3.5 text-left hidden md:table-cell">Celular</th>
              <th className="px-4 py-3.5 text-left cursor-pointer hover:opacity-100 opacity-80 select-none hidden lg:table-cell w-32" onClick={() => toggleSort('dataInclusao')}>
                Dt. Inclusão <SortIcon col="dataInclusao" />
              </th>
              <th className="px-4 py-3.5 text-center w-28">Ações</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((m, i) => (
              ...
              >
                ...
                <td className="px-4 py-3.5 text-base tabular-nums hidden sm:table-cell" style={{ color: 'var(--adm-muted)' }}>{m.rg}</td>
                ...
                <td className="px-4 py-3.5 text-base hidden md:table-cell" style={{ color: 'var(--adm-muted)' }}>{m.celular}</td>
                <td className="px-4 py-3.5 text-base hidden lg:table-cell" style={{ color: 'var(--adm-subtle)' }}>{m.dataInclusao}</td>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div
            className="w-full max-w-2xl rounded-2xl shadow-2xl border max-h-[90vh] overflow-y-auto"
          ...
          >
            ...
            <div className="px-6 py-5 grid grid-cols-2 gap-4">
              ...
              {/* Data de Inclusão */}
              <div>
                <label className="block text-sm font-semibold mb-1.5" style={{ color: 'var(--adm-muted)' }}>Data de Inclusão</label>
                {isRO ? (
                  <input readOnly value={form.dataInclusao} className={inputCls()} style={roStyle} />
                ) : (
                  <input
                    type="date"
                    value={toISO(form.dataInclusao)}
                    onChange={e => changeField('dataInclusao', toBR(e.target.value))}
                    className={inputCls()}
                    style={fieldStyle}
                  />
                )}
              ...
              {/* Celular */}
              <div>
                <label className="block text-sm font-semibold mb-1.5" style={{ color: 'var(--adm-muted)' }}>Celular</label>
                <input
                  type="text"
                  value={form.celular}
                  onChange={e => changeField('celular', e.target.value)}
                  readOnly={isRO}
                  placeholder="(00) 00000-0000"
                  className={inputCls(errors.celular)}
                  style={isRO ? roStyle : { ...fieldStyle, ...(errors.celular ? { borderColor: '#ef4444' } : {}) }}
                />
                {errors.celular && (
                  <p className="text-sm mt-1 text-red-400 flex items-center gap-1">
                    <AlertCircle size={13} /> {errors.celular}
                  </p>
                )}
              </div>
              ...
              {/* WhatsApp */}
              <div>
                <label className="block text-sm font-semibold mb-1.5" style={{ color: 'var(--adm-muted)' }}>WhatsApp</label>
                <input
                  type="text"
                  value={form.whatsapp}
              ...
              <div className="col-span-2">
                ...
                {isRO ? (
                  ...
                  <select
                    ...
                    style={fieldStyle}
                  >
                    {POSTOS_GRADUACOES_CHAMADA.map(p => <option key={p} value={p}>{p}</option>)}
                  </select>
                )}
              </div>
              ...
              {/* Endereço */}
              <div className="col-span-2">
                <label className="block text-sm font-semibold mb-1.5" style={{ color: 'var(--adm-muted)' }}>Endereço Completo</label>
                <textarea
                  value={form.endereco}
