...
const emptyForm = (): Omit<ViaturaItem, 'id'> => ({
  ordem: null,
  tombamento: 'N/C',
  estado: 'BOM',
  quantidade: 1,
...
export default function ViaturasModule({ onBack, permissions }: Props) {
  ...
  const [modal, setModal] = useState<{ mode: ModalMode; item: ViaturaItem | null } | null>(null);
  const [form, setForm] = useState<Omit<ViaturaItem, 'id'>>(emptyForm());
  const [errors, setErrors] = useState<{ especificacao?: string; quantidade?: string }>({});
  const [deleteTarget, setDeleteTarget] = useState<ViaturaItem | null>(null);
  ...
  const changeField = (key: keyof Omit<ViaturaItem, 'id'>, value: string) => {
    setForm(current => ({
      ...current,
      [key]: key === 'quantidade'
  ...
  const openView = (item: ViaturaItem) => {
    setForm(item);
    setErrors({});
    setModal({ mode: 'view', item });
  };

  const openEdit = (item: ViaturaItem) => {
    setForm(item);
    setErrors({});
  ...
  const handleSave = () => {
    ...
    if (modal?.mode === 'create') {
      setData(current => [{ ...form, id: `viatura-${Date.now()}` }, ...current]);
  ...
  const exportCSV = () => {
    const header = 'Ordem,Tombamento,Estado de Conservação,Quantidade,Especificação,Valor,Localização';
    ...
    dl('viaturas.csv', 'text/csv;charset=utf-8', `\uFEFF${header}\n${rows.join('\n')}`);
  };

  const exportJSON = () => dl('viaturas.json', 'application/json', JSON.stringify(filtered, null, 2));

  const exportXLSX = () => {
    const wb = XLSX.utils.book_new();
    const ws = XLSX.utils.json_to_sheet(exportRows);
    ws['!cols'] = [{ wch: 8 }, { wch: 16 }, { wch: 22 }, { wch: 12 }, { wch: 72 }, { wch: 12 }, { wch: 30 }];
  ...
  const exportPrint = () => {
    const rows = filtered.map(item => `
      <tr>
        <td>${html(item.ordem)}</td><td>${html(item.tombamento)}</td><td>${html(item.estado)}</td>
        <td>${html(item.quantidade)}</td><td>${html(item.especificacao)}</td><td>${html(item.valor)}</td><td>${html(item.localizacao)}</td>
      </tr>
    `).join('');
    const doc = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Viaturas 31ª CIPM-CPE</title>
      ...
      <h3>Viaturas - 31ª CIPM-CPE</h3><div class="meta">${filtered.length} registros | Quantidade total: ${totalQuantidade}</div>
      <table><thead><tr><th>Ordem</th><th>Tombamento</th><th>Estado</th><th>Qtd.</th><th>Especificação</th><th>Valor</th><th>Localização</th></tr></thead>
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex flex-wrap items-center gap-3 mb-6">
        ...
        <h3 className="font-bold text-2xl flex-1" style={{ color: 'var(--adm-text)' }}>Viaturas</h3>
        <span className="text-base" style={{ color: 'var(--adm-muted)' }}>{filtered.length} registros | {totalQuantidade} itens</span>
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          placeholder="Buscar por ordem, tombamento, especificação, estado, valor ou localização..."
        ...
        <table className="w-full min-w-[980px]">
          <thead className="sticky top-0 z-10" style={{ background: 'var(--adm-tbl-head)' }}>
            <tr className="text-sm font-semibold uppercase tracking-wide" style={{ color: 'var(--adm-muted)' }}>
              <th className="px-4 py-3.5 text-left cursor-pointer select-none" onClick={() => toggleSort('ordem')}>Ordem <SortIcon col="ordem" /></th>
          ...
          <tbody>
            {paginated.map((item, i) => (
              <tr key={item.id} className="adm-row border-t transition-colors" style={{ borderColor: 'var(--adm-border)', background: i % 2 === 0 ? 'var(--adm-row-even)' : 'transparent' }}>
                <td className="px-4 py-3.5 text-base tabular-nums" style={{ color: 'var(--adm-subtle)' }}>{item.ordem ?? '-'}</td>
                <td className="px-4 py-3.5 text-base whitespace-nowrap" style={{ color: 'var(--adm-muted)' }}>{item.tombamento}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="w-full max-w-3xl rounded-2xl shadow-2xl border" style={{ background: 'var(--adm-modal)', borderColor: 'var(--adm-border)' }}>
            <div className="flex items-center justify-between px-6 py-4 border-b" style={{ borderColor: 'var(--adm-border)' }}>
              <h4 className="font-bold text-xl" style={{ color: 'var(--adm-text)' }}>{modal.mode === 'create' ? 'Nova Viatura' : modal.mode === 'edit' ? 'Editar Registro' : 'Visualizar Registro'}</h4>
              <button onClick={() => setModal(null)} className="transition-colors" style={{ color: 'var(--adm-muted)' }}><X size={22} /></button>
            </div>

            <div className="px-6 py-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-semibold mb-1.5" style={{ color: 'var(--adm-muted)' }}>Ordem</label>
                <input type="number" value={form.ordem ?? ''} onChange={e => changeField('ordem', e.target.value)} readOnly={isRO} className={inputCls()} style={isRO ? roStyle : fieldStyle} />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="number" min={1} value={form.quantidade} onChange={e => changeField('quantidade', e.target.value)} readOnly={isRO} className={inputCls(errors.quantidade)} style={isRO ? roStyle : { ...fieldStyle, ...(errors.quantidade ? { borderColor: '#ef4444' } : {}) }} />
              ...
              <div className="sm:col-span-2">
                ...
                <textarea value={form.especificacao} onChange={e => changeField('especificacao', e.target.value)} readOnly={isRO} rows={4} className="adm-input w-full rounded-lg px-3 py-2.5 text-base border resize-none" style={isRO ? roStyle : { ...fieldStyle, ...(errors.especificacao ? { borderColor: '#ef4444' } : {}) }} />
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm rounded-2xl shadow-2xl border p-6" style={{ background: 'var(--adm-modal)', borderColor: 'var(--adm-border)' }}>
            ...
            <p className="text-base mb-6" style={{ color: 'var(--adm-muted)' }}>Deseja remover <span className="font-semibold" style={{ color: 'var(--adm-text)' }}>{deleteTarget.especificacao}</span>?</p>
